...
export function getUniqueSymbols(sequences: string[]) {
    return sequences
        .join('')
        .split('')
        .filter(function(symbol, index, allSymbols) {
            return allSymbols.indexOf(symbol) === index;
        })
        .sort()
        .join('');
}
...
export const getDividedSubstitutionMatrixByOccurrences = (matrix: (number | '-')[][], occurrencesOfSymbols: number[]) => {
    return matrix.map((row, indexOfSymbol) => {
        return row.map(cell => {
            if (cell === '-') return cell;
            else return cell / occurrencesOfSymbols[indexOfSymbol];
        });
    });
};
