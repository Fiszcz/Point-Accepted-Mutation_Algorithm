...
export const SubstitutionMatrix: React.FC<SubstitutionMatrixProps> = ({ step }) => {
    ...
    const matrixWithDividedValuesByOccurrences = useSelector(
        (state: AppState) => state.substitutionMatrix.matrixWithDividedValuesByOccurrences,
    );
                                        ...
                                        >
                                            <Typography size={25 - widthOfTable}>
                                                {[4, 6, 7, 8].includes(step || 0) && cell !== '-' ? (
                                                    ...
                                                    indexOfColumn === 0 || indexOfColumn === uniqueListOfSymbols.length - 1 ? (
                                                        <div className={css({ '& > *': { fontSize: 15 } })}>
                                                            {React.createElement(Latex, null, '$1-\\sum_{j<>i}M_{ij}$')}
                                                        </div>
                                                    ) : (
                                                        '-||-'
                                                    )
                                                ...
                                                {step && indexOfColumn !== indexOfRow && [2, 3, 6].includes(step || 0) && (
                                                    <>
                                                        ...
                                                        {step === 2 ? (
                                                            <>
                                                                N<sub>{uniqueListOfSymbols[indexOfRow]}</sub>
                                                            </>
                                                        ) : step === 3 ? (
                                                            occurrencesNumberOfSymbols[indexOfRow]
                                                        ) : step === 6 ? (
                                                            React.createElement(Latex, null, '$\\lambda$')
                                                        ) : (
                                                            ''
                                                        )}
                                                    </>
